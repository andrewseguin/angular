...
function addScript(options: Schema) {
  return (host: Tree, context: SchematicContext) => {
    ...
    try {
      ...
      // Handle the old json - .angular-cli.json
      const angularCliJsonFile = host.read('.angular-cli.json');
      if (angularCliJsonFile) {
        const json = JSON.parse(angularCliJsonFile.toString('utf-8'));
